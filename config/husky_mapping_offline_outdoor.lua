...
TRAJECTORY_BUILDER_3D.high_resolution_adaptive_voxel_filter.max_range = 55.
...
TRAJECTORY_BUILDER_3D.motion_filter.max_angle_radians = math.rad(0.0)

-- Submaps --
TRAJECTORY_BUILDER_3D.submaps.num_range_data = 80
TRAJECTORY_BUILDER_3D.submaps.high_resolution = 0.6
TRAJECTORY_BUILDER_3D.submaps.high_resolution_max_range = 65.
...
-- Constraint search --
POSE_GRAPH.max_local_constraint_distance = 60.
POSE_GRAPH.local_constraints_per_node = 0.1
...
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher_3d.linear_xy_search_window = 20.0  -- 5.0
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher_3d.linear_z_search_window = 15.0  -- 1.0
POSE_GRAPH.constraint_builder.fast_correlative_scan_matcher_3d.angular_search_window = math.rad(30.0)  -- math.rad(15.0)
...
POSE_GRAPH.matcher_translation_weight = 500  -- 5e2
POSE_GRAPH.matcher_rotation_weight = 1600  -- 1.6e3
POSE_GRAPH.optimization_problem.local_slam_pose_translation_weight = 2e4  -- 1e5
POSE_GRAPH.optimization_problem.local_slam_pose_rotation_weight = 2e4  -- 1e5
...
TRAJECTORY_BUILDER.loop_trimmer = {
  ...
  translation_to_rotation_error = math.rad(1),
  translation_error_rate = 0.05,
  rotation_to_translation_error_rate = 0.01 / (2 * math.pi),

  trim_close_loops = false,
